...
###
    ...
    def __transforms(self, image_A, image_B, label):
        ...
        if self.aug:
            if self.rescale_range:
                image_A, image_B, label = transforms.random_scaling(
                    image_A, image_B, label=label,
                    scale_range=self.rescale_range)

            if self.img_fliplr:
                image_A, image_B, label = transforms.random_fliplr(image_A, image_B, label=label)
            ...
            ### 弱监督变化检测，不能用随机裁剪 ###
            if self.crop_size:
                image_A, image_B, label, img_box = transforms.random_crop(
                    image_A, image_B, label=label,
    ...
    def __getitem__(self, idx):   # img_box for transformational samples
        ...
        image_A, image_B, label, img_box = self.__transforms(image_A=image_A, image_B=image_B, label=label)
        ...
        if self.aug:
            return img_name, image_A, image_B, cls_label, label, img_box
        else:
            return img_name, image_A, image_B, cls_label, label
