...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x_a, x_b):
        ...
        x_a = self.prepare_tokens(x_a)
        x_a = self.pos_drop(x_a)
        embeds_a = []
        x_b = self.prepare_tokens(x_b)
...
class VisionTransformer_with_attn(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def forward_features(self, x):
        ...
        return x[:, 0], x[:, 1:], embeds[self.aux_layer][:, 1:], torch.cat(attns[-2:], dim=1)
...
class VisionTransformer_memo(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def forward_features(self, x, proto, layer=3):
        ## see self.prepare_tokens
        # B = x.shape[0]
        # x = self.patch_embed(x)
        # cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        ...
        embeds = []
        attns = []
        proto_num = proto.shape[1]

        # proto: (21, 768)
        for i, blk in enumerate(self.blocks):
            if i == layer:  # last layer
                x = torch.cat([x, proto], dim=1)  # integrate prototype
                x, weights = blk(x, )
                embeds.append(x)
        ...
        x = self.norm(x)
        embeds[-1] = x
        return x[:, 0], x[:, 1:-proto_num], embeds[self.aux_layer][:, 1:-proto_num], attns[-1]
...
class VisionTransformer_mct(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + num_classes, embed_dim))
        self.pos_embed.requires_grad = False
        self.pos_drop = nn.Dropout(p=drop_rate)
        self._size = img_size // patch_size
        self.patch_size = patch_size
    ...
    def prepare_tokens(self, x):
        ...
        patch_pos_embed = patch_pos_embed.reshape(1, self._size, self._size, -1).permute(0, 3, 1, 2)
        patch_pos_embed = F.interpolate(patch_pos_embed, size=(h, w), mode="bicubic", align_corners=False)
        patch_pos_embed = patch_pos_embed.reshape(1, -1, h * w).permute(0, 2, 1)
        pos_embed = torch.cat((self.pos_embed[:, :self.num_classes, :], patch_pos_embed), dim=1)
    ...
    def forward_features(self, x):
        ...
        return x[:, 0:self.num_classes], x[:, self.num_classes:], embeds[self.aux_layer][:, self.num_classes:], attns[
            -3]
...
class VisionTransformer_mask(nn.Module):
    ...
    def prepare_tokens(self, x, mask=None):
        ...
        if mask is not None:
            ...
            k = mask.flatten(1).unsqueeze(-1).type_as(mask_token)
            x = x * k + mask_token * (1 - k)
...
class VisionTransformer_mask_v1(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
...
class VisionTransformer_with_aff(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
